                        ...
                        engine::VoxelPalette& palette)
{
    ...
    const std::vector<engine::VolumeSpec> specs = {
        // instances 0 and 1 correspond to balls A and B in the frame update.
        // the volume origin is 0.12 m below its center on each axis.
        {"SphereA", {24, 24, 24}, {-0.62f, 1.88f, -0.12f},
         engine::VoxelVolume::FLAG_DYNAMIC, {0.01f, 0.01f, 0.01f}},
        {"SphereB", {24, 24, 24}, {0.38f, 1.88f, -0.12f},
         engine::VoxelVolume::FLAG_DYNAMIC, {0.01f, 0.01f, 0.01f}},
    ...
    data.push_back(buildSphereVolume(specs[0].dims, kMatSphereA));
    data.push_back(buildSphereVolume(specs[1].dims, kMatSphereB));
    data.push_back(buildBoxVolume(specs[2].dims, kMatFloor));
    data.push_back(buildBoxVolume(specs[3].dims, kMatWall));
    data.push_back(buildBoxVolume(specs[4].dims, kMatWall));
